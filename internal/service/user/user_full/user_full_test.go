...
import (
	...
	modelUserCategories "github.com/WagaoCarvalho/backend_store_go/internal/model/user/user_category"
	modelsUserCatRel "github.com/WagaoCarvalho/backend_store_go/internal/model/user/user_category_relation"
	modelsUserContactRel "github.com/WagaoCarvalho/backend_store_go/internal/model/user/user_contact_relation"
...
func TestUserFullService_CreateFull(t *testing.T) {
	...
	t.Run("Relations: sucesso ao criar relações user-contact e user-category", func(t *testing.T) {
		...
		mockRepoUserContactRel.On("CreateTx", ctx, tx, mock.AnythingOfType("*model.UserContactRelation")).
			Return(&modelsUserContactRel.UserContactRelation{
				UserID:    createdUser.UID,
				ContactID: userFull.Contact.ID,
				CreatedAt: time.Now(),
			}, nil)
		mockRepoUserCatRel.On("CreateTx", ctx, tx, mock.AnythingOfType("*model.UserCategoryRelation")).
			Return(&modelsUserCatRel.UserCategoryRelation{
				UserID:     createdUser.UID,
				CategoryID: 100,
				CreatedAt:  time.Now(),
			}, nil)
	...
	t.Run("UserContactRelation: falha ao criar relação no repo", func(t *testing.T) {
		...
		mockRepoUserContactRel.On(
			"CreateTx", ctx, tx, mock.AnythingOfType("*model.UserContactRelation"),
