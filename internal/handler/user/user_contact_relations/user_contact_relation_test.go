...
import (
	...
	model "github.com/WagaoCarvalho/backend_store_go/internal/model/user/user_contact_relations"
...
func setupUserContact() (*mockUser.MockUserContactRelationService, *UserContactRelationHandler) {
...
func TestUserContactRelationHandler_Create(t *testing.T) {
	t.Run("success - relação criada", func(t *testing.T) {
		...
		expectedModel := &model.UserContactRelations{
			UserID:    1,
			ContactID: 2,
			CreatedAt: time.Now(),
		}
...
func TestUserContactRelationHandler_GetAllByUserID(t *testing.T) {
	t.Run("success - retorna relações", func(t *testing.T) {
		...
		expectedRelations := []*model.UserContactRelations{
			{
				UserID:    1,
				ContactID: 10,
				CreatedAt: time.Now(),
