...
import (
	...
	model "github.com/WagaoCarvalho/backend_store_go/internal/model/supplier/supplier_contact_relations"
...
func setupSupplierContact() (*mockSupplier.MockSupplierContactRelationService, *SupplierContactRelationHandler) {
...
func TestSupplierContactRelationHandler_Create(t *testing.T) {
	t.Run("success - relação criada", func(t *testing.T) {
		...
		expectedModel := &model.SupplierContactRelations{
			SupplierID: 1,
			ContactID:  2,
			CreatedAt:  time.Now(),
		}
	...
	t.Run("already exists - relação já existente", func(t *testing.T) {
		...
		existingModel := &model.SupplierContactRelations{
			SupplierID: 1,
			ContactID:  2,
			CreatedAt:  time.Now(),
		}
...
func TestSupplierContactRelationHandler_GetAllBySupplierID(t *testing.T) {
	t.Run("success - relações retornadas", func(t *testing.T) {
		...
		supplierID := int64(1)
		relations := []*model.SupplierContactRelations{
