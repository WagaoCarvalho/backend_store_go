...
func TestClientHandler_Create(t *testing.T) {
	...
	t.Run("Success - Create Client", func(t *testing.T) {
		...
		cpf := "12345678900"
		inputDTO := &dto.ClientCpfDTO{
			Name:  "Cliente Teste",
			Email: email,
			CPF:   cpf,
		}

		expectedModel := dto.ToClientCpfModel(*inputDTO)
		expectedModel.ID = 1

		mockService.On("Create", mock.Anything, mock.MatchedBy(func(m *models.ClientCpf) bool {
			return m.Name == inputDTO.Name &&
				m.Email == inputDTO.Email &&
				m.CPF == inputDTO.CPF
	...
	t.Run("Error - Invalid Foreign Key", func(t *testing.T) {
		...
		email := "fk@teste.com"
		inputDTO := &dto.ClientCpfDTO{Name: "Cliente FK", Email: email}

		mockService.On("Create", mock.Anything, mock.Anything).
			Return((*models.ClientCpf)(nil), errMsg.ErrDBInvalidForeignKey)
	...
	t.Run("Error - Duplicate Client", func(t *testing.T) {
		...
		inputDTO := &dto.ClientCpfDTO{Name: "Duplicado", Email: email}

		mockService.On("Create", mock.Anything, mock.Anything).
			Return((*models.ClientCpf)(nil), errMsg.ErrDuplicate)
	...
	t.Run("Error - Service Failure", func(t *testing.T) {
		...
		inputDTO := &dto.ClientCpfDTO{Name: "Falha", Email: email}

		mockService.On("Create", mock.Anything, mock.Anything).
			Return((*models.ClientCpf)(nil), assert.AnError)
...
func TestClientHandler_Update(t *testing.T) {
	...
	t.Run("erro - ID zero (ErrZeroID)", func(t *testing.T) {
		...
		clientWithZeroID := &models.ClientCpf{ID: 0, Name: "Cliente X"}
	...
	t.Run("erro - conflito de versão (ErrVersionConflict)", func(t *testing.T) {
		...
		clientWithVersion := &models.ClientCpf{ID: 1, Name: "Cliente Y", Version: 1}
	...
	t.Run("erro - service retorna ErrNotFound", func(t *testing.T) {
		...
		inputDTO := &dto.ClientCpfDTO{Name: "Cliente Teste"}
		body, _ := json.Marshal(inputDTO)

		mockService.On("Update", mock.Anything, mock.MatchedBy(func(c *models.ClientCpf) bool {
	...
	t.Run("sucesso - update", func(t *testing.T) {
		...
		mockService.On("Update", mock.Anything, mock.MatchedBy(func(m *models.ClientCpf) bool {
			return m.Name == inputDTO.Name &&
				m.Email == inputDTO.Email &&
				m.CPF == inputDTO.CPF &&
				m.ID == 1
	...
	t.Run("erro - service retorna outro erro", func(t *testing.T) {
		...
		uid := "1"
		reqBody := &dto.ClientCpfDTO{Name: "Cliente Teste"}
...
func TestClientHandler_Delete(t *testing.T) {
	...
	t.Run("erro - id inválido", func(t *testing.T) {
		mockService := new(mockClient.MockClientCpf)
		handler := NewClientCpfHandler(mockService, logger)
	...
	t.Run("Erro - Delete - cliente não encontrado", func(t *testing.T) {
		t.Parallel()
