...
func TestClientHandler_Filter(t *testing.T) {
	...
	// Teste com filtro de CNPJ
	t.Run("sucesso - filtro por CNPJ", func(t *testing.T) {
		mockService := new(mockClient.MockClient)
		handler := NewClientFilterHandler(mockService, logger)
		...
		mockService.
			On("Filter", mock.Anything, mock.MatchedBy(func(f *filter.ClientFilter) bool {
				return f.CNPJ == "12345678000199"
			})).
			Return(mockClients, nil)

		req := httptest.NewRequest(http.MethodGet, "/clients/filter?cnpj=12345678000199", nil)
		rec := httptest.NewRecorder()
...
func TestClientHandler_ParseTimeParam(t *testing.T) {
	...
	handler := &clientFilterHandler{logger: logger}
