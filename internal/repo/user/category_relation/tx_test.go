...
func TestNewUserCategoryRelationTx(t *testing.T) {
	t.Run("successfully create new UserCategoryRelationTx instance", func(t *testing.T) {
		var db repo.DBExecutor

		result := NewUserCategoryRelationTx(db)

		assert.NotNil(t, result)

		_, ok := result.(*userCategoryRelationTx)
		assert.True(t, ok, "Expected result to be of type *userCategoryRelationTx")
	})
	...
	t.Run("return different instances for different calls", func(t *testing.T) {
		...
		instance1 := NewUserCategoryRelationTx(db)
		instance2 := NewUserCategoryRelationTx(db)
...
func TestUserCategoryRelationTx_CreateTx(t *testing.T) {
	t.Run("successfully create user category relation within transaction", func(t *testing.T) {
		mockTx := new(mockDb.MockTx)
		repo := &userCategoryRelationTx{db: nil}
		...
		mockTx.On("Exec", ctx, mock.Anything, []interface{}{relation.UserID, relation.CategoryID}).Return(mockResult, nil)
	...
	t.Run("return ErrRelationExists when duplicate key (unique violation)", func(t *testing.T) {
		...
		mockTx.On("Exec", ctx, mock.Anything, []interface{}{relation.UserID, relation.CategoryID}).
