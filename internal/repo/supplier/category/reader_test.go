...
func TestSupplierCategoryRepo_GetByID(t *testing.T) {
	t.Run("successfully get supplier category by id", func(t *testing.T) {
		...
		categoryDescription := "Electronic components and devices"
		...
		mockDB.
			...
			Return(&mockDb.MockRowWithIDArgs{
				Values: []any{
					...
					categoryDescription, // description - string
		...
		assert.Equal(t, categoryDescription, result.Description)
...
func TestSupplierCategoryRepo_GetAll(t *testing.T) {
	t.Run("successfully get all supplier categories", func(t *testing.T) {
		mockDB := new(mockDb.MockDatabase)
		repo := &supplierCategoryRepo{db: mockDB}
		ctx := context.Background()
	...
	t.Run("successfully get multiple supplier categories", func(t *testing.T) {
		mockDB := new(mockDb.MockDatabase)
		repo := &supplierCategoryRepo{db: mockDB}
		ctx := context.Background()
	...
	t.Run("successfully get empty list when no supplier categories exist", func(t *testing.T) {
		mockDB := new(mockDb.MockDatabase)
		repo := &supplierCategoryRepo{db: mockDB}
		ctx := context.Background()

		mockRows := new(mockDb.MockRows)
		// Primeira chamada de Next() já retorna false - nenhuma linha
		mockRows.On("Next").Return(false).Once()
		mockRows.On("Err").Return(nil)
		mockRows.On("Close").Return()
		...
		assert.Empty(t, result)  // Deve estar vazia
