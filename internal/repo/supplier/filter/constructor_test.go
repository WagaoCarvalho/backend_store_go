...
func TestNewSupplier(t *testing.T) {
	t.Run("successfully create new supplier instance", func(t *testing.T) {
		mockDB := new(mockDb.MockDatabase)

		result := NewFilterSupplier(mockDB)
	...
	t.Run("return different instances for different calls", func(t *testing.T) {
		...
		instance1 := NewFilterSupplier(mockDB)
		instance2 := NewFilterSupplier(mockDB)
